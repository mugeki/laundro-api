...
// Create provides a mock function with given fields: orderData, productData
func (_m *Repository) Create(orderData *orders.Domain, productData *products.Domain) (orders.Domain, error) {
	ret := _m.Called(orderData, productData)

	var r0 orders.Domain
	if rf, ok := ret.Get(0).(func(*orders.Domain, *products.Domain) orders.Domain); ok {
		r0 = rf(orderData, productData)
	} else {
		r0 = ret.Get(0).(orders.Domain)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(*orders.Domain, *products.Domain) error); ok {
		r1 = rf(orderData, productData)
	} else {
		r1 = ret.Error(1)
	}
